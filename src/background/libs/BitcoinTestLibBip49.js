...
class BitcoinTestLibClass{
    constructor(wallet){
        ...
        this.networks = wallet.networks.BTC49TESTNETWORK
    }

    getBalance(raw=true){
        return new Promise(async(resolve,reject)=>{
            try{
                let address = await this.generateAddAndPriv.generateAddress("BTC49TEST");
    ...
    sendTransaction(to,amount,fee){
  	...
  	createSignRawTx(to, amount,fee){
    	return new Promise(async(resolve,reject)=>{
            ...
            let privKey = await this.generateAddAndPriv.generatePrivKey("BTC49TEST");
            this.validator.validateString(privKey);
            let keyring = await bitcoin.ECPair.fromWIF(privKey,this.networks);
            let p2wpkh = bitcoin.payments.p2wpkh({ pubkey: keyring.publicKey, network: this.networks })
            let p2sh = bitcoin.payments.p2sh({ redeem: p2wpkh, network: this.networks })
			let txb = new bitcoin.TransactionBuilder(this.networks)
			let from = await this.generateAddAndPriv.generateAddress("BTC49TEST");
                ...
                if(balance >= amount+fee){
                    ...
                    let change = tmpSum - amount - fee;
                        if (change > 0){
                            ...
                            change = this.toDecimals(change)
                            alert("Insufficient balance: trying to send "+amount+" BTC + "+fee+" BTC fee when having "+change+" BTC at the moment because you have unconfirmed transactions")
                        }
	                }else{
                        amount = this.toDecimals(amount)
                        fee = this.toDecimals(fee)
